...
async function lihatDetail() {
  try {
    ...
    if (!container || !rekomen) {
      console.log("Container detailBuku atau rekomenBuku tidak ditemukan");
      return;
    }
  ...
  <button
    onclick="window.location.href='../dist/login.html'"
  ...
  <button
    ...
    const rekomendasi = produk.filter(
      (buku) =>
        buku.Category === item.Category && String(buku.Id) !== String(item.Id),
    );
