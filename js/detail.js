...
async function lihatDetail() {
  try {
    ...
    rekomen.innerHTML = rekomendasi
      .map(
        (buku) => `
          <div class="w-[150px] h-[400px] bg-white border border-gray-200 overflow-hidden flex flex-col transition-all duration-300 hover:-translate-y-2 hover:shadow-xl mb-12">
            <a href="./detail.html?id=${buku.Id}">
              <img
                src="${buku.image}"
                alt="${buku.Judul}"
                class="w-full h-[210px] object-cover flex-shrink-0"
              >
            </a>

            <div class="px-3 py-3 flex flex-col flex-1">
              <h3 class="text-[13px] font-bold line-clamp-2 min-h-[38px]">
                ${buku.Judul}
              </h3>

              <p class="text-[11px] italic text-slate-900 line-clamp-1 mt-1 min-h-[16px]">
              ...
              <div class="mt-auto pt-4">
                <a
                  ...
                  class="block w-full py-2 bg-slate-900 rounded-2xl text-white text-[10px] font-semibold text-center hover:bg-[#184e67] transition duration-300"
                >
                  Read Now
                </a>
              </div>
